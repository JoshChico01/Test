...
export const RadioActiveFlagged: FC<Props> = memo(function RadioActiveFlagged(props = {}) {
  return (
    <div className={`${resets.clapyResets} ${props.classes?.root || ''} ${props.className || ''} ${classes.root}`}>
      <div className={classes.radio}>{props.swap?.radio || <RadioIcon className={classes.icon} />}</div>
      {props.text?.amount != null ? props.text?.amount : <div className={classes.amount}>yes</div>}
    </div>
  );
});
